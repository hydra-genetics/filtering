...
rule bgzip_vcf:
    input:
        "filtering/{file}.vcf",
    output:
        temp("filtering/{file}.vcf.gz"),
    log:
        "filtering/{file}.vcf.gz.log",
    benchmark:
        repeat("filtering/{file}.vcf.gz.benchmark.tsv", config.get("bgzip_vcf", {}).get("benchmark_repeats", 1))
    ...
    conda:
        "../envs/bgzip_vcf.yaml"
    message:
        "{rule}: bgzip filtering/{wildcards.file}.vcf"
    shell:
