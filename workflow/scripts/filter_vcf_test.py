...
class TestUnitUtils(unittest.TestCase):
    ...
    def test_soft_and_hard_filtering(self):
        ...
        with open(vcf, 'w', encoding="ascii") as out_vcf:
            filter_variants("^([A-Za-z0-9-]+_[RT]{1})$", self.in_vcf, out_vcf, ".tests/unit/config_filter_format_and_vep_hard_and_soft_unittest_1.yaml")
        ...
        with open(vcf, 'w', encoding="ascii") as out_vcf:
            filter_variants("^([A-Za-z0-9-]+_[RN]{1})$", self.in_vcf, out_vcf, ".tests/unit/config_filter_format_and_vep_hard_and_soft_unittest_1.yaml")
        with VariantFile(vcf) as variants:
            test_table = {
        ...
        with open(vcf, 'w', encoding="ascii") as out_vcf:
            self.assertRaises(Exception, filter_variants, "^([A-Za-z0-9-]+_[W]{1})$", self.in_vcf, out_vcf, ".tests/unit/config_filter_format_and_vep_hard_and_soft_unittest_1.yaml")

        with open(vcf, 'w', encoding="ascii") as out_vcf:
            filter_variants("^([A-Za-z0-9-]+_[RN]{1})$", self.in_vcf, out_vcf, ".tests/unit/config_filter_format_and_vep_hard_and_soft_unittest_2.yaml")
        with VariantFile(vcf) as variants:
            test_table = {
    ...
    def test_handle_missing_data(self):
        ...
        vcf = os.path.join(tempdir, "test.vcf")
        with open(vcf, 'w', encoding="ascii") as out_vcf:
            filter_variants("^([A-Za-z0-9-]+_[RN]{1})$", self.in_vcf, out_vcf, ".tests/unit/config_filter_format_missing_value_unittest_1.yaml")
        with VariantFile(vcf) as variants:
            test_table = {
        ...
        with open(vcf, 'w', encoding="ascii") as out_vcf:
            filter_variants("^([A-Za-z0-9-]+_[T]{1})$", self.in_vcf, out_vcf, ".tests/unit/config_filter_format_missing_value_unittest_2.yaml")
        with VariantFile(vcf) as variants:
            test_table = {
                            "chr1:934486-934487": "SB_mutect2",
                            "chr1:935221-935222": "SB_mutect2",
        ...
        with self.assertRaises(ValueError):
            filter_variants("^([A-Za-z0-9-]+_[RN]{1})$", self.in_vcf, out_vcf, ".tests/unit/config_filter_format_missing_value_unittest_3.yaml")
